  ...
  return (
    <section aria-labelledby="rates-h" className="bg-paper py-20 md:py-28">
      <div className="gutter">
        <SectionTag n="01" name="The rate card" className="mb-8" />

        <div className="grid grid-cols-1 gap-8 md:grid-cols-12 md:items-end">
          <h1 id="rates-h" className="d-1 font-serif text-ink md:col-span-7">
            <Words lines={['The stay', 'they deserve.']} italicLine={1} flameLine={1} />
          </h1>
        ...
        <div className="mt-14 grid grid-cols-1 gap-4 md:mt-20 md:grid-cols-2 lg:grid-cols-3">
          {pricingCategories.map((c, i) => (
            <motion.a
              ...
              className="group flex flex-col justify-between rounded-3xl border border-ink/15 bg-bone p-7 shadow-[0_20px_50px_-32px_rgba(27,22,19,0.4)] transition-colors duration-500 hover:border-flame/60"
            >
              <div>
                <span className="font-sans text-[0.75rem] tabular-nums text-flame">{c.n}</span>
                <h2 className="mt-2 font-serif text-[clamp(1.4rem,2.4vw,1.85rem)] leading-tight text-ink transition-colors duration-300 group-hover:text-flame">
                  {c.title}
                </h2>
              ...
              <span className="mt-6 flex items-center gap-2 font-sans text-[0.7rem] font-bold uppercase tracking-[0.16em] text-ink">
                ...
                <ArrowRight size={14} className="text-flame transition-transform duration-300 group-hover:translate-x-1" />
              </span>
            </motion.a>
          ))}
        </div>
